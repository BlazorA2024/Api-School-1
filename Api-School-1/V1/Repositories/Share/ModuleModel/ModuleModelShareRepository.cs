...
{
    /// <summary>
    /// ModuleModel class for ShareRepository.
    /// </summary>
    public class ModuleModelShareRepository : BaseShareRepository<ModuleModelRequestShareDto, ModuleModelResponseShareDto, ModuleModelRequestBuildDto, ModuleModelResponseBuildDto>, IModuleModelShareRepository
    {
        // Declare the builder repository.
        private readonly ModuleModelBuilderRepository _builder;
        /// <summary>
        /// Constructor for ModuleModelShareRepository.
        /// </summary>
        public ModuleModelShareRepository(DataContext dbContext, IMapper mapper, ILoggerFactory logger) : base(mapper, logger)
        {
            // Initialize the builder repository.
            _builder = new ModuleModelBuilderRepository(dbContext, mapper, logger.CreateLogger(typeof(ModuleModelShareRepository).FullName));
        ...
        /// </summary>
        public override async Task<ModuleModelResponseShareDto> CreateAsync(ModuleModelRequestShareDto entity)
        {
            try
            {
        ...
        /// </summary>
        public override async Task<IEnumerable<ModuleModelResponseShareDto>> GetAllAsync()
        {
            try
            {
        ...
        /// </summary>
        public override async Task<ModuleModelResponseShareDto?> GetByIdAsync(string id)
        {
            try
            {
        ...
        /// </summary>
        public override IQueryable<ModuleModelResponseShareDto> GetQueryable()
        {
            try
            {
                _logger.LogInformation("Retrieving IQueryable<ModuleModelResponseShareDto> for ModuleModel entities...");
        ...
        public Task SaveChangesAsync()
        {
            try
            {
                _logger.LogInformation("Saving changes to the database for ModuleModel entities...");
                throw new NotImplementedException();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in SaveChangesAsync for ModuleModel entities.");
        ...
        /// </summary>
        public override async Task<ModuleModelResponseShareDto> UpdateAsync(ModuleModelRequestShareDto entity)
        {
            try
            {
        ...
        public override async Task<PagedResponse<ModuleModelResponseShareDto>> GetAllAsync(string[]? includes = null, int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                return new PagedResponse<ModuleModelResponseShareDto>(items, results.PageNumber, results.PageSize, results.TotalPages);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error while fetching all ModuleModels.");
                return new PagedResponse<ModuleModelResponseShareDto>(new List<ModuleModelResponseShareDto>(), pageNumber, pageSize, 0);
            }
        }

        public override async Task<ModuleModelResponseShareDto?> GetByIdAsync(object id)
        {
            try
            {
        ...
        public override async Task DeleteRange(List<ModuleModelRequestShareDto> entities)
        {
            try
            {
                var builddtos = entities.OfType<ModuleModelRequestBuildDto>().ToList();
        ...
        public override async Task<PagedResponse<ModuleModelResponseShareDto>> GetAllByAsync(List<FilterCondition> conditions, ParamOptions? options = null)
        {
            try
            {
                _logger.LogInformation("[Share]Retrieving  ModuleModel entities as pagination...");
                return MapToPagedResponse(await _builder.GetAllByAsync(conditions, options));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Share]Error in GetAllByAsync for ModuleModel entities as pagination.");
                return null;
            }
        }

        public override async Task<ModuleModelResponseShareDto?> GetOneByAsync(List<FilterCondition> conditions, ParamOptions? options = null)
        {
            try
            {
                _logger.LogInformation("[Share]Retrieving ModuleModel entity...");
                return MapToShareResponseDto(await _builder.GetOneByAsync(conditions, options));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Share]Error in GetOneByAsync  for ModuleModel entity.");
                return null;
            }
        }
    }
